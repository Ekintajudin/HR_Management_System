...
include __DIR__ .'/function/claimProc.php';
...
//claim
//read table claim
$app->get('/claim', function (Request $request, Response $response, array $arg){

    return $this->response->withJson(array('data' => 'success'), 200); });  
 
// read all data from table claim 
$app->get('/allclaim',function (Request $request, Response $response,  array $arg) { 

    $data = getAllclaim($this->db); 
    if (is_null($data)) { 
...
//request table order by condition (claim id) 
...
//CLAIM !!!
//post method order
$app->post('/claim/add', function ($request, $response, $args) { 

    $form_data = $request->getParsedBody(); 
    $data = createclaim($this->db, $form_data); 
    if (is_null($data)) { 
...
//claim !!!
//delete row Order
$app->delete('/claim/del/[{id}]', function ($request, $response, $args){   
    $claimId = $args['id']; 
    
   if (!is_numeric($claimId)) { 

       return $this->response->withJson(array('error' => 'numeric paremeter required'), 422); } 
       $data = deleteclaim($this->db,$claimId); 
       if (empty($data)) { 

           return $this->response->withJson(array($claimId=> 'is successfully deleted'), 202);}; }); 
...
//claim
//put table order 
$app->put('/claim/put/[{id}]', function ($request, $response, $args){
    $claimId = $args['id']; 
    ...
    if (!is_numeric($claimId)) { 
        ...
        return $this->response->withJson(array('error' => 'numeric paremeter required'), 422); } 
        $form_dat=$request->getParsedBody(); 
        $data=updateclaim($this->db,$form_dat,$claimId); 
        if ($data <=0)
        return $this->response->withJson(array('data' => 'successfully updated'), 200); 
});